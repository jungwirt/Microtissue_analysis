...
```{r}
microtissue_sct <- microtissue

DefaultAssay(microtissue_sct) <- "RNA"

microtissue_sct <- SCTransform(microtissue_sct) |>
  RunPCA() |>
  FindNeighbors() |>
  RunUMAP(dims = 1:50) |>
...
```{r}
clustree::clustree(microtissue_sct@meta.data[,grep("SCT_snn_res", colnames(microtissue_sct@meta.data))],
                   prefix = "SCT_snn_res.")
...
```{r}
colnames(microtissue@meta.data)
...
png("umap_harmony_v2.png", width = 1000, height = 800)
p2
dev.off()
...
```{r}
DimPlot(microtissue, reduction = "umap", shuffle = T, group.by = c("Sample"), ncol = 1, label=T, pt.size = .1) + ggtitle("SCTransform with harmony") -> p3 

png("sample_harmony_clustered.png", width = 1000, height = 800)
p3
dev.off()
...
```{r}
microtissue$harmony_snn_res.0.1[microtissue$harmony_snn_res.0.1 == 4] <- 1
...
write.csv(Markers, here::here(output_dir_data, "Microtissue_harmony_01.csv"))

Markers_top10 <- as.data.frame(Markers %>% group_by(cluster) %>% top_n(n = 10, wt = avg_log2FC))
write.csv(Markers_top10, here::here(output_dir_data, "Microtissue_harmony_01_top10.csv"))

Markers_top3 <- as.data.frame(Markers %>% group_by(cluster) %>% top_n(n = 3, wt = avg_log2FC))
write.csv(Markers_top3, here::here(output_dir_data, "Microtissue_harmony_01_top3.csv"))
```
...
Heatmap <- DoHeatmap(subset(microtissue, cells = sample(colnames(microtissue), 14000)), label = FALSE, draw.line = F, features = Markers_top10$gene, assay = "SCT") +
  scale_fill_gradientn(colours = heatmap_cols) +
  theme(axis.text.y = element_text(size = 5)) +
  theme(plot.margin = unit(c(0.1, 0, 0, 0), 
                           "inches"))

png("heatmap_clustered_harmony_microtissue.png", width = 1000, height = 800)
Heatmap
dev.off()
...
```{r}
saveRDS(microtissue_sct, 
        here::here(output_dir_data, "microtissue_sct.rds"))
...
```{r}
saveRDS(microtissue, 
        here::here(output_dir_data, "microtissue_harmony.rds"))

```

```{r}
integrated <- readRDS(here::here(output_dir_data, "microtissue_harmony.rds"))
#unintegrated <- readRDS(here::here(output_dir_data, "microtissue_sct.rds"))
```


```{r}
png("markers_clustered_harmony_featureplot.png", width = 1000, height = 800)
FeaturePlot_scCustom(integrated, features = c("TNNI3K", "RYR2", "FGF12", "MYH6",
                                              "DCN",  "COL1A1", "FN1", "ACTA2", 
                                              "COMP", "POSTN", "VWF", "PECAM1", 
                                              "CDH5" , "CD34", "TAGLN", "CNN1", 
                                              "MYH11", "MYLK", "LMOD1", "ACTG2"), num_columns = 4)
dev.off()
```
